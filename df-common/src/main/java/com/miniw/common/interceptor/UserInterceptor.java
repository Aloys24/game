...
import com.miniw.common.utils.CommonUtil;
...
//        return true;
    ...
    private boolean checkForRequestParam(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if(!redisUtil.hHasKey(RedisKeyConstant.USER_LOGIN_DATA, uinStr)){
            redisUtil.hPut(RedisKeyConstant.USER_LOGIN_DATA, uinStr, JSON.toJSONString(userLoginData));
        }
        return true;
    }
    ...
    private void sendMsg(HttpServletResponse response) throws IOException {
        ResultMsg resultMsg = new ResultMsg(ResultCode.SYS_NEED_LOGIN.getCode(), ResultCode.SYS_NEED_LOGIN.getMsg());
...
//        response.setStatus(HttpStatus.UNAUTHORIZED.value());
    ...
    void verifyFailMsg(HttpServletResponse response) throws IOException {
        ResultMsg resultMsg = new ResultMsg(ResultCode.SYS_PARAM_ERROR.getCode(), ResultCode.SYS_PARAM_ERROR.getMsg());
        response.setCharacterEncoding(StandardCharsets.UTF_8.name());
